 ...
 */
// TODO: to be removed, using it for testing
public class ECMStatusGenerator extends RequestGenerator {

    public byte[] generateRequest(byte sequenceID, int boxID) {
        // 13 for header, 4 for boxID  and 9 for timestamp
        byte[] ba = new byte[13 + 4 + 9];
        ...
        //TODO: change to 'C' for the new protocol
        ba[11] = (byte)'D';
        ...
        System.arraycopy(yrBytes, 0, ba, index, 2); // 2 bytes
        ...
        ba[index++] = (byte)cal.get(Calendar.SECOND);
        //  byte[] msBytes = ConnectionUtils.intToByte(cal.get(Calendar.MILLISECOND), 2);
        int tmp=0;
        byte[] msBytes = ConnectionUtils.intToByte(tmp, 2);
        System.arraycopy(msBytes, 0, ba, index, 2); // 2 bytes
